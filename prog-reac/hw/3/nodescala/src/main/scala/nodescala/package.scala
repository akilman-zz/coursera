...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs.foldRight(Future(Nil: List[T]))((f, acc) => 
        for {
          t <- f 
          ts <- acc
        } yield (t :: ts))
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]
      fs.map(f => f.onComplete(t => p.tryComplete(t)))
      p.future
    }
    ...
    def delay(t: Duration): Future[Unit] = {
      Future { Try(blocking { Await.ready(never, t) })}
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      f.value match {
        ...
        case None => throw new NoSuchElementException("No value available")
      }
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      ...
      f.onComplete { 
        ...
        case Success(v) => p.complete(Try(cont(Future.always(v))))
      }
      p.future
    }
