...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    ...
    while (response.hasNext && !token.isCancelled) {		// while there is more data to process and not cancelled
      val next = response.next                              // get the 'next' chunk
      exchange.write(next)									// write to the exchange entity
    }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listenerSubscriber = listener.start
    
    val requestSubscriber = Future.run() { cancellationToken =>
      Future {
        while (cancellationToken.nonCancelled) {
          ...
          nextRequest.onComplete(
              t => { // t: Try((Request, Exchange))
                val request = t.get._1
                val exchange = t.get._2
    ...
    Subscription.apply(listenerSubscriber, requestSubscriber)
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      // async handler to complete the promise
      createContext(exchange => p.success((exchange.request, exchange)))
       
      // deregister once complete
      val f = p.future
      f.onComplete(_ => removeContext)
      
      // return the future with the request
      f
    }
  }
