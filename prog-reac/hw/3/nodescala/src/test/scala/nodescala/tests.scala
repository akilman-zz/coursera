...
class NodeScalaSuite extends FunSuite {
  ...
  test("All") {
    ...
    assert(Await.result(all, 1 second) === 1 :: 2 :: 3 :: Nil)
  }
  ...
  test("Delay") {
	  ...
	  assert(Await.result(delay, 2 seconds) === BoxedUnit.UNIT)
  }
  ...
  test("Now") {
    ...
    try {
      ...
      case t: NoSuchElementException => // expected!
    }
  }
  
  test("continueWith") {
    val f1 = Future.always(42)
    val f2 = Future.always(42)
    
    val f1andf2 = f1.continueWith(_ => f2)
    val next = Await.result(f1andf2, 1 second) 
    assert(next != null)
    
    assert(Await.result(next, 1 second) === 42)
  }
  
  test("continue") {
    val f = Future.always(42)
    val f1 = f.continue(t => 19)
    
    val f1Result = Await.result(f1, 1 second)
    assert(f1Result === 19)
  }
