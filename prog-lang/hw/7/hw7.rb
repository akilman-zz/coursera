...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult segment
    ...
    x1 = xs.min - GeometryExpression::Epsilon
    x2 = xs.max + GeometryExpression::Epsilon
    
    ys = [segment.y1, segment.y2]
    y1 = ys.min - GeometryExpression::Epsilon
    y2 = ys.max + GeometryExpression::Epsilon
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult segment
    _x1 = [x1, segment.x1].max
    _x2 = [x2, segment.x2].min
    _y1 = [y1, segment.y1].max
    _y2 = [y2, segment.y2].min

    if _x1 > _x2 or _y1 > _y2
      NoPoints.new
    else
      LineSegment.new(_x1, _y1, _x2, _y2)
    end
  end
