...
(* 
  Helper function used to perform the actual shift of the expression
*)
fun do_shift (dx, dy, e) = 
    case e of 
            NoPoints => e
          ...
          | _ => raise Impossible("Unable to shift expression")   
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Intersect(e1,e2) => intersect(eval_prog(e1,env),
                                      eval_prog(e2, env))
      | Shift (dx, dy, e) => 
        (case e of
             Var s => do_shift(dx, dy, eval_prog(e, env))
          | _ => do_shift(dx, dy, e))
...
fun preprocess_prog e = 
    case e of 
     ...
     | Let(s, e1, e2) => Let(s, preprocess_prog(e1),
                             preprocess_prog(e2))
