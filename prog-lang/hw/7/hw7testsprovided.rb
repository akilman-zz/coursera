...
if not ((b4.is_a? Point) and b4.x == ZERO and b4.y == FIVE)
	puts "Line intersect with point not working"
end

b5 = b.intersect(Point.new(ZERO, FOUR))
if not ((b5.is_a? NoPoints))
	puts "Line intersect with bogus point not working"
end

line1 = Line.new(ONE, ZERO)
line2 = Line.new(-ONE, ZERO)
intersection = line1.intersect(line2)
if not ((intersection.is_a? Point) and intersection.x == ZERO and intersection.y == ZERO)
	puts "Two lines intersecting does not work"
end

line1 = Line.new(ONE, ZERO)
line2 = Line.new(-ONE, TWO)
intersection = line1.intersect(line2)
if not ((intersection.is_a? Point) and intersection.x == ONE and intersection.y == ONE)
	puts "Two lines intersecting does not work"
end
...
c4 = c.intersect(Line.new(ONE, THREE))
if not ((c4.is_a? Point) and c4.x == THREE and c4.y == SIX)
	puts "VerticalLine intersecting with Line not working"
end
...
s1 = LineSegment.new(0, 0, 5, 5)
s2 = LineSegment.new(0, 5, 5, 0)
p = s1.intersect(s2)
if not ((p.is_a? Point) and p.x == 2.5 and p.y == 2.5)
	puts "LineSegment test ain't working properly"
end
...
s = Shift.new(0, 0, Point.new(0, 0))
s1 = s.preprocess_prog.eval_prog([])	
if not (s1.x == 0 and s1.y == 0)
	puts "Shifting point not working properly"
end

s = Shift.new(1, 1, Point.new(0, 0))
s1 = s.preprocess_prog.eval_prog([])
if not (s1.x == 1 and s1.y == 1)
	puts "Shifting point by nonzero amount not working"
end

s = Shift.new(1, 1, VerticalLine.new(3))
s1 = s.preprocess_prog.eval_prog([])
if not (s1.x == 4)
	puts "Shifting vertical line not working properly"
end

s = Shift.new(1, 1, Line.new(1, 0))
s1 = s.preprocess_prog.eval_prog([])
if not (s1.m == 1 and s1.b == 1)
	puts "Shifting regular line not working properly"
end
